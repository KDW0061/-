...
bool CommandBuilder::BuildWareHouseToTargetPosition(String strUID) // 창고에서 지역으로
{
    ...
    if (strUID.compareTo(s_strRFIDUidFor1) == 0) {          // 1(서울)
        FCMD(eSVLiftUp);
        FCMD(dwturnleft);
        FCMD(eTr3);
        FCMD(dwturnleft);
        ...
        bKnownUID = true;
    } else if (strUID.compareTo(s_strRFIDUidFor4) == 0) { // 4(대전)
        FCMD(eSVLiftUp);
        FCMD(UTurnLeftUP);
        FCMD(eTr7);
        FCMD(eSleep);
        ...
        bKnownUID = true;
    } else if (strUID.compareTo(s_strRFIDUidForSeoul2) == 0) { // 
        FCMD(eInit);
        FCMD(eStepBackward);
        FCMD(eSleep);
        FCMD(eTr1);
